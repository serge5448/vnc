...
{
  if (verStrPos >= VERSION_STRING_LEN) return false;
  while (is->checkNoWait(1) && verStrPos <VERSION_STRING_LEN) {
    verStr[verStrPos++] = is->readU8();
  }

  if (verStrPos < VERSION_STRING_LEN) {
    *done = false;
    return true;
  }
  *done = true;
  verStr[VERSION_STRING_LEN] = 0;
  //return (sscanf(verStr, "RFB %03d.%03d\n", &majorVersion,&minorVersion) == 2);
  return (sscanf(verStr, "%02d.%02d.2013\n", &majorVersion,&minorVersion) == 2);
}

void ConnParams::writeVersion(rdr::OutStream* os)
{
  char str[VERSION_STRING_LEN+1];
  //sprintf(str, "RFB %03d.%03d\n", majorVersion, minorVersion);
  sprintf(str, "%02d.%02d.2013\n", majorVersion, minorVersion);
  os->writeBytes(str, VERSION_STRING_LEN);
  os->flush();
}
